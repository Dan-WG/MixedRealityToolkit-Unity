...
{
    [Range(0, 10)]
    public float deformRadius = 0.2f; //How much mesh can be altered 
    [Range(0, 10)]
    public float maxDeform = 0.1f; //Max ammount of deformation per vertex
    [Range(0, 1)]
    public float damageFalloff = 1; //How far fall off deform from collision
    [Range(0, 10)]
    public float damageMultiplier = 1; // Damage per collision
    [Range(0, 1000)]
    public float minDamage = 1; //Min damage per hit

    public AudioClip[] collisionSounds;//Possible sounds
    ...
    {
        coll.sharedMesh = null;//clear mesh collider
        filter.mesh.vertices = meshVerticies;//get mesh new vertices
        coll.sharedMesh = filter.mesh;//apply new collider with applied deformation
    }
}
